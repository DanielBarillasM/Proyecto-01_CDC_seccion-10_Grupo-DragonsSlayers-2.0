...
`null` puede asignarse a referencias de instancia y arreglos. Los tipos primitivos (`integer`, `float`, `boolean`, `string`) no reciben `null` implícitamente.
...
Los diagnósticos semánticos usan códigos `SEM001` a `SEM021`. Los IDs internos (`sem-diag-N`) se reinician en cada ejecución para que los resultados y pruebas sean deterministas.
